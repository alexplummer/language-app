...
var tinyTermsApp = function () {
        ...
        clickListener(revealBtn, function (i) {
            ...
            if (revealGoalCount === tinyTerms$1[tinyTerms$1.pickedList].ops.revealGoalTarget) {
                ...
                var gameWon = function gameWon() {
                    ...
                    // Award bonus points
                    var completeBonus = Object.keys(tinyTerms$1[tinyTerms$1.pickedList].terms).length * 10;
                    tinyTerms$1[tinyTerms$1.pickedList].storedData.score += completeBonus;
                    setScore();
                };
    ...
    var showHome = function showHome() {
        ...
        localforage.getItem('tinyTerms.uploadedLists', function (err, uploadedLists) {
            ...
            function buildList(thisBtn) {
                ...
                var categoryType = thisBtn.innerHTML;
                lists = "";
                for (var _val in tinyTerms$1.listChoices) {

                    if (tinyTerms$1.listChoices[_val].category === categoryType) {
                        lists += '<a href="#">' + tinyTerms$1.listChoices[_val].name + '</a>';
                    }
                }
                for (var _val2 in tinyTerms$1.customListChoices) {

                    if (tinyTerms$1.listChoices[_val2].category === categoryType) {
                        lists += '<div class="' + makeSafeClass(tinyTerms$1.customListChoices[_val2].name) + '"><span class="custom-list"></span><a href="#">' + tinyTerms$1.customListChoices[_val2].name + '</a></div>';
                    }
                }
            }
        ...
        function navListeners() {
            ...
            var _loop2 = function _loop2(_i2) {
                ...
                removeList[_i2].addEventListener('click', function (e) {
                    e.preventDefault();
                    var insertPoint = removeList[_i2].nextSibling;
                    document.querySelector('.create-list').classList.add('hidden');
                    removeList[_i2].classList.add('hidden');
                    removeList[_i2].parentNode.insertBefore(document.createElement("div"), insertPoint.nextSibling);
                    ...
                    document.querySelector('.delete-confirm').addEventListener('click', function () {
                        var listName = removeList[_i2].parentNode.childNodes[1].innerHTML;
                        ...
                        localforage.setItem('tinyTerms.uploadedLists', tinyTerms$1.uploadedLists, function () {
                            removeList[_i2].parentNode.removeChild(insertPoint.nextSibling);
                            removeList[_i2].parentNode.parentNode.removeChild(document.querySelector('.' + makeSafeClass(listName)));
                    ...
                    document.querySelector('.delete-cancel').addEventListener('click', function () {
                        removeList[_i2].parentNode.removeChild(insertPoint.nextSibling);
                        removeList[_i2].classList.remove('hidden');
                        document.querySelector('.create-list').classList.remove('hidden');
                    });
                });
            };

            for (var _i2 = 0; _i2 < removeList.length; _i2++) {
                _loop2(_i2);
            }
        }
            ...
            if (inputName.length > 0 && inputSheet.length > 0) {
                ...
                if (checkURL(inputSheet) === true) {
                    localforage.getItem('tinyTerms.uploadedLists', function (err, uploadedLists) {
                        ...
                        localforage.setItem('tinyTerms.uploadedLists', tinyTerms$1.uploadedLists, function () {
                            hideModal(true);
                            var customlist = '<div class="' + makeSafeClass(inputName) + '"><span class="custom-list"></span><a href="#">' + inputName + '</a></div>';
                            var homeWrapper = document.querySelector('.m-menu');
    ...
    // Imports
    // Show home screen
    var onboardShow$1 = function onboardShow() {
        var modal = document.querySelector('.m-modal');
    ...
    var navMenu = function navMenu() {
        ...
        function hideNav() {
            nav.querySelector('.close').addEventListener('click', function (e) {
                e.preventDefault();
                hideNav();
            });
        ...
        localforage.getItem("tinyTermsAllPicked", function (err, tinyTermsAllPicked) {
            ...
            for (var _i3 = 0; _i3 < tinyTerms$1.AllPicked.length; _i3++) {
                navHTML += '<a href="#">' + tinyTerms$1.AllPicked[_i3] + '</a>';
            }
    ...
    var initialise = function initialise() {
        // Error handling for Tabletop
        errorAlert('Couldn\'t load the list, please try again.', function () {
